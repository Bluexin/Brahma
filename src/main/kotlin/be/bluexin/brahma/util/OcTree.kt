...
package be.bluexin.brahma.util

import be.bluexin.brahma.bag
import be.bluexin.brahma.getPool
import be.bluexin.brahma.plusAssign
...
class OcTree @JvmOverloads constructor(
    ...
    /**
     * Public constructor for [ObjectPool] use only
     */
    ...
    /**
     * Returns entity ids of entities that are inside [QuadTree]s that contain given point and have the given flags set
     *
     *
     * Returned entities must be filtered further as these results are not exact
     *
    ...
    /**
     * Returns entity ids of entities that are inside [QuadTree]s that overlap given bounds
     *
     *
     * Returned entities must be filtered further as these results are not exact
     */
    ...
    /**
     * Returns entity ids of entities that are inside [QuadTree]s that overlap given bounds and have the given flags set
     *
     *
     * Returned entities must be filtered further as these results are not exact
     *
    ...
    operator fun get(
        ...
        if (bounds.overlaps(x, y, z, width, height, depth)) {
            if (nodes[0] != null) {
                val index = indexOf(x, y, z, width, height, depth)
                if (index != QuadTree.OUTSIDE) nodes[index]!![fill, x, y, z, width, height, depth, flags]
    ...
    fun getExact(
        ...
        if (bounds.overlaps(x, y, z, width, height, depth)) {
            if (nodes[0] != null) {
                val index = indexOf(x, y, z, width, height, depth)
                if (index != QuadTree.OUTSIDE) nodes[index]!!.getExact(fill, x, y, z, width, height, depth)
